...
//   lPoles.clear();
//   facets.clear();
//   Delaunay_hierarchy crust(delaunay);
...
//   std::map<Delaunay::Cell_handle,bool> tested_cell;
...
// 	cell != cells.end();
// 	++cell) {
//       tested_cell[*cell] = false;
//       if(!delaunay.is_infinite(*cell)) {
// 	poles.push_back(delaunay.dual(*cell));
//       }
//     }
//     assert(!poles.empty());
//     std::sort(poles.begin(), poles.end(), lessPoint(iter->point()));
//     Point pole(poles.back());

//     CGAL::Angle angle = CGAL::RIGHT;
//     std::vector<Point>::reverse_iterator iter_poles = poles.rbegin();
...
// 	++iter_poles) {
//       angle = CGAL::angle(pole, iter->point(), *iter_poles);
...
//       }
//     }
//     adder result_obtu = for_each(obtus.begin(), obtus.end(), adder());
//     adder result_aigu = for_each(aigus.begin(), aigus.end(), adder());

//     Point obtus_average;
//     Point aigu_average;
    
//     if(!obtus.empty()) {
//       obtus_average = Point(result_obtu.x / obtus.size(), 
// 			    result_obtu.y / obtus.size(), 
// 			    result_obtu.z / obtus.size());
//     }
//     if(!aigus.empty()) {
//       aigu_average = Point(result_aigu.x / aigus.size(), 
// 			   result_aigu.y / aigus.size(), 
// 			   result_aigu.z / aigus.size());
//     }
...
//     Delaunay::Vertex_handle v0 = vertices_match[c->vertex( (i+1)&3 )];
//     Delaunay::Vertex_handle v1 = vertices_match[c->vertex( (i+2)&3 )];
//     Delaunay::Vertex_handle v2 = vertices_match[c->vertex( (i+3)&3 )];
//     Delaunay::Cell_handle cell;
//     int l, j, k;
//     if(crust.is_facet (v0, v1, v2, cell, l, j, k)) {
...
//   std::cout << "Comp facets=" << temps_cpu << " Size=" << facets.size() << std::endl;
// }
